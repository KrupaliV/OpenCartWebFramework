...
- Location: tests\homepage.spec.ts:17:1

# Error details

```
TypeError: homePage.getHomePageTitle is not a function
```
...
```ts
  ...
  13 |     await loginPage.doLogin('test6769078698@gmail.com', 'Test@1234');
  14 |     homePage = new HomePage(page);
  15 | });
  16 | 
  17 | test('home page title test', async () => {
> 18 |     const pageTitle = await homePage.getHomePageTitle();
     |                                      ^ TypeError: homePage.getHomePageTitle is not a function
  ...
  22 | 
  23 | 
  24 | test('logout link exist test', async () => {
  25 |     expect(await homePage.isLogoutLinkExist()).toBeTruthy();
  26 | });
  27 | 
  28 | 
  29 | test('home page headers exist test @junesprint', async () => {
  30 |     let allHeaders = await homePage.getHomePageHeaders();
  31 |     console.log('home page headers: ', allHeaders);
  32 |     expect.soft(allHeaders).toHaveLength(4);
  33 |     expect.soft(allHeaders).toEqual([
  34 |         'My Account',
  35 |         'My Orders',
  36 |         'My Affiliate Account',
  37 |         'Newsletter'
  38 |     ]);
  39 | });
  40 | 
